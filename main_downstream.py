...
def train_multitask(model, df, optimizer, loss_fn_dict, graphs, task_names, per_task_output_units_num):
    ...
    for i in range(0, len(idx), batch_size):
        ...
        output = model(atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask,
                       x, edge_index, edge_weight, batch_tensor)
...
fingerprint_model = Fingerprint(radius, T, x_atom.shape[-1], x_bonds.shape[-1], fingerprint_dim, output_units_num, p_dropout)
